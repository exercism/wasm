...
beforeAll(async () => {
  try {
    const watPath = new URL("./tournament.wat", import.meta.url);
...
const tournamentTally = (matches) => {
  const input = matches + '\n';
  const inputBufferOffset = 64;
  const inputBufferCapacity = 384;
  ...
  const [outputOffset, outputLength] = currentInstance.exports.tournamentTally(
