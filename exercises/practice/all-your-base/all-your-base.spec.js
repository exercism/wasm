...
beforeAll(async () => {
  try {
    const {buffer} = await compileWat("all-your-base.wat", {multi_value: true});
...
function convert(digits = [], inputBase, outputBase){
  ...
  const inputBuffer = new Int32Array(linearMemory.buffer, inputOffset, digits.length);
  inputBuffer.set(digits, 0);

  // Pass offset and length to WebAssembly function
  let [outputOffset, outputLength, rc] = currentInstance.exports.convert(inputOffset, digits.length, inputBase, outputBase);

  const outputBuffer = new Int32Array(linearMemory.buffer, outputOffset, outputLength);
