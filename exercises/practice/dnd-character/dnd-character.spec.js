...
beforeAll(async () => {
  try {
    const watPath = new URL("./dnd-character.wat", import.meta.url);
...
const abilityModifier = (constitution) => {
  const modifier = currentInstance.exports.abilityModifier(constitution);
  if (modifier === -99999) throw new Error('Ability scores must be at least 3');
  if (modifier === 99999) throw new Error('Ability scores can be at most 18');
  return modifier;
}

class Character {
  constructor() {
    const [outputOffset, outputLength] = currentInstance.exports.createCharacter();
    const attributes = currentInstance.get_mem_as_i32(outputOffset, outputLength);
    Object.assign(this, attributes.reduce((character, value, index) => {
      character[Character.values[index]] = value;
      return character;
    }, {}));
  }
  static values = ['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma', 'hitpoints'];
  static rollAbility() { return currentInstance.exports.rollAbility(); }
}
