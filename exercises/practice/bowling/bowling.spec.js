...
beforeAll(async () => {
  try {
    const watPath = new URL("./bowling.wat", import.meta.url);
...
const status = {
  "0": "OK",
  "-1": "Negative roll is invalid",
  "-2": "Pin count exceeds pins on the lane",
  "-3": "Cannot roll after game is over",
  "-4": "Score cannot be taken until the end of the game",
};

class Bowling {
  roll(points) {
    const exitCode = currentInstance.exports.roll(points);
    if (exitCode) throw new Error(status[exitCode]);
  }
  score() {
    const score = currentInstance.exports.score();
    if (score < 0) throw new Error(status[score]);
    return score;
  }
}
